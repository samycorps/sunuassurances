var Payment = function() {
    return {
        ...
        init: function() {
             ...
             */
            _this.getExistingVehicleDetailsByProfile().then(values => {
                console.log(values);
                if (values.length > 0) {
                    values.map(v => {
        ...
        displayPaymentDetails: function() {
            ...
            const KOBO_NAIRA_DIVISION = 1000;
            $.each(_this.fields.transactionDetails, (i,v) => {
                ...
                const markup = `<tr>
                    ...
                    <td>${v.cover_type}</td>
                    <td>${v.expiry_date}</td>
                    <td><button class="btn btn-primary" onclick="_this.printPage(${i})"><i class="fa fa-print"></button></td>
                </tr>`;
