var Motor = function() {
    return {
        ...
        setEffectiveAndExpiryDates: () => {
            ...
            const expiry_date = moment().add(1, 'years').subtract(1, 'days').format('YYYY-MM-DD');
        ...
        setActive: () => {
            ...
            // if (event.target.id === 'additionalPolicy') {
            //     $('#existingPolicyDiv').removeClass('hide_elements');
            //     _this.fields.coverOption = 'additional';
            // } else {
        ...
        loadDropdownLists: () => {
            Promise.all([Motor.getColoursList(), Motor.getVehicleBodyList(), Motor.getCoverTypeList(), Motor.getSectorList(), Motor.getStateList(), Motor.getRiskClassList(), Motor.getBankList(), Motor.getCompanyBankList(), Motor.getVehicleModelList()]).then(values => {
                console.log(values);
                [Motor.fields.colours, Motor.fields.vehicleBodies, Motor.fields.coverTypes, Motor.fields.sectors, Motor.fields.states, Motor.fields.riskClasses, Motor.fields.banks, Motor.fields.companyBanks, _this.fields.vehicleModels] = values;
            ...
            _this.getIndividualPolicyList().then(result => {
                _this.fields.individualPolicyList = result;
                if (!_.isEmpty(_this.fields.individualPolicyList)) {
                    $('#newPolicy').addClass('hide_elements');
                    $('#rootwizardPanel').addClass('hide_elements');
                }
            },
            error => {
                console.log(error);
        ...
        wizardStepOne: () => {
            ...
            // else if(!$('#newAndAdditionalPolicySection').hasClass('hide_elements')) {
            //     _this.getPolicyEnquiryDetails().then(result => {
            //         // console.log(result);
            //         $('.loading_icon').addClass('hide_elements');
            //         _thisBroker.fillVehicleDetails(result.message);
            ...
            // }
                ...
                _this.getExistingVehicleDetails()
            .then(result => {
                if (_.isEmpty(result)) {
        ...
        wizardStepTwo: () => {
            ...
            if ($('#tab2form').valid()) {
                ...
                if (!_.isEmpty(_this.fields.savedEntry)) {
                    vehicleData['vehicleDetailsId'] = _this.fields.savedEntry.id;
                }
                Utility.saveVehicleDetails(operation, vehicleData).then(result => {
                    console.log(result);
        ...
        wizardStepThree: () => {
            ...
            const mobileNumber = Utility.fields.selectedProfile.gsm_number.trim() !== '' ? Utility.fields.selectedProfile.gsm_number : Utility.fields.selectedProfile.office_number;
        ...
        wizardStepFour: () => {
            ...
            if ($('#renewPolicy').hasClass('active')) {
                promise = _thisBroker.getRenewalPolicy(Motor.legendData);
            }
            else {
        ...
        fetchVehicleDetails: () => {
            Motor.getVehicleDetails()
                .then(function(result) {
                    ...
                    $xml = $( xmlDoc );
                    responseString = $xml.find( "string" ).text().replace(/\n|\r/g, "");
                    ...
                    Motor.fields.vehicleDetails = {
                        licenseInfo: {
                            ...
                            isssueDate: $innerXml.find( "IsssueDate" ).text().replace('T', ' '),
                            expiryDate: $innerXml.find( "ExpiryDate" ).text().replace('T', ' ')
                        },
                        response: {
                    ...
                    if (Motor.fields.vehicleDetails.response && Motor.fields.vehicleDetails.response.responseMessage === "ok") {
                        ...
                        $('.alert-message').addClass('error');
                        $('.alert-message-text').html(Motor.fields.vehicleDetails.response.responseMessage || 'error fetching vehicle details');
                    }
                })
                .catch(error => {
                    console.log(error);
        ...
        setLegendData: () => {
            ...
            const data = {
                ...
                lga: 'L17001',
                tin_number: Utility.fields.selectedProfile.tin_number === '' ? 'N/A' : Utility.fields.selectedProfile.tin_number,
                bvn: '09876543211',
        ...
        saveVehicleDetails: (vehicleTransactionDetails) => {
            const url = _this.fields.vehicleDetails.otherDetails !== undefined ? `${api_urls.vehicletransactiondetails}/${_this.fields.vehicleDetails.otherDetails.vehicleDetailsId}` : api_urls.vehicletransactiondetails;
        ...
        onClassChange: () => {
            ...
            const vehicle_purchase_price = $('#vehicle_purchase_price').val().replace(/,/g, '');
            const vehicle_value = vehicle_purchase_price && (!isNaN(vehicle_purchase_price)) ? parseFloat(vehicle_purchase_price).toFixed(2) : 0.00;
            ...
            switch (insurance_class) {
                case 'comprehensive': {
                    ...
                    if($('#comprehensive_message').hasClass('hide')) {
                        ...
                        $('#third_party_message').removeClass('hide').addClass('hide') 
                    }
                    ...
                    if($('#comprehensive_addons_tracking').hasClass('hide') && vehicle_purchase_price > tracking_display_threshold) {
                    ...
                    if (comprehensive_type === 'quartely') {
                        ...
                        expiry_date = moment().add(parseInt(numDaysInYear/4), 'days').format('YYYY-MM-DD');
                        $('#vehicle_expiry_date').val(expiry_date); 
                    }
                    else if (comprehensive_type === 'half_year') {
                        coverage_amount = coverage_amount / 2;
                        expiry_date = moment().add(parseInt(numDaysInYear/2), 'days').format('YYYY-MM-DD');
                        $('#vehicle_expiry_date').val(expiry_date); 
                    }
                    else {
                        expiry_date = moment().add(1, 'years').subtract(1, 'days').format('YYYY-MM-DD');
                        $('#vehicle_expiry_date').val(expiry_date); 
                    }
                ...
                case 'third_party': {
                    ...
                    if($('#third_party_message').hasClass('hide')) {
                        ...
                        $('#comprehensive_message').removeClass('hide').addClass('hide')
                    }
        ...
        populateColorList: () => {
            ...
            .filter(function() { return $.trim( $(this).text() ) == Motor.fields.vehicleDetails.licenseInfo.color.toUpperCase(); })
        ...
        populateVehicleModels: () => {
            ...
            .filter(function() { return $.trim( $(this).text() ) == Motor.fields.vehicleDetails.licenseInfo.model.toUpperCase(); })
            .attr('selected','selected');
        },
        ...
        printPage: (index) => {
            const policyDetails = _this.fields.individualPolicyList[index];
            ...
            request.onload = function() {
                ...
                reader.onload =  function(e){
                    _this.getExistingVehicleDetailsById(policyDetails.vehicle_transaction_details_id)
                    .then(result => {
                        const effectiveDate = moment(policyDetails.expiry_date.substring(0,9), 'DD-MMM-YY').subtract(1, 'year').format('DD-MMM-YY');
...
var MotorBroker = function() {
    return {
        ...
        setLegendRenewalData: () => {
            ...
            expiry_date = moment(effective_date,'YYYY-MM-DD').add(1, 'years').format('YYYY-MM-DD');
            ...
            const data = {
                ...
                model: _thisBroker.fields.renewalDetails.makeModel,
